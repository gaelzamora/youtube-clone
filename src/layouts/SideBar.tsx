import { ChevronDown, ChevronUp, Clapperboard, Clock, Home, Library, PlaySquare, Repeat, History, ListVideo, Icon, Flame, ShoppingBag, Music2, Film, Radio, Gamepad2, Newspaper, Trophy, Lightbulb, Shirt, Podcast } from 'lucide-react'
...
export function SideBar() {
    ...
    return (
        <>   
            ...
            {isSmallOpen && (
                <div onClick={close} className='lg:hidden fixed inset-0 z-[999] bg-secondary-dark opacity-50 left-0' />
            )}
            <aside className={`w-56 lg:sticky absolute top-0 overflow-y-auto scrollbar-hidden pb-4 flex-col gap-2 px-2 ${isLargeOpen ? "lg:flex" : "lg:hidden"} ${isSmallOpen ? "flex z-[999] bg-white max-h-screen" : "hidden"}`}>
                ...
                <LargeSidebarSection visibleItemCount={5}>
                    ...
                    <LargeSidebarItem IconOrImgUrl={PlaySquare} title='Your Videos' url='/yourvideos' />
                ...
                <LargeSidebarSection title='Subscriptions'>
                    {subscriptions.map(subscription => (
                        <LargeSidebarItem key={subscription.id} IconOrImgUrl={subscription.imgUrl} title={subscription.channelName} url='/library' />
                    ))}
                </LargeSidebarSection>
                <hr />
...
function LargeSidebarSection({children, title, visibleItemCount = Number.POSITIVE_INFINITY}: LargeSidebarSectionProps) {
    ...
    return (
        <div>
            {title && <div className='ml-4 mt-2 text-lg mb-1 text-left'>{title}</div>}
            {visibleChildren}
            {showExpandButton && 
...
function LargeSidebarItem({IconOrImgUrl, title, url, isActive = false}: LargeSidebarItemProps) {
    return (
        <a href={url} className={twMerge(buttonStyles({
            variant: "ghost"}), `w-full flex items-center rounded-lg gap-4 p-3 ${isActive ? "font-bold bg-neutral-100 hover:bg-secondary" : undefined}`)}>
